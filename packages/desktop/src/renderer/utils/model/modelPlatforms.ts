 ...
 * Platform type
 */
export type PlatformType = 'gemini' | 'gemini-vertex-ai' | 'anthropic' | 'custom' | 'new-api' | 'bedrock' | 'dual-route';
...
export interface PlatformConfig {
  ...
  base_url?: string;
  /** 国际�?key（可选，用于需要翻译的平台名称�?/ i18n key (optional, for platform names that need translation) */
  i18nKey?: string;
}
...
export const MODEL_PLATFORMS: PlatformConfig[] = [
    // ˫·��ģʽ / Dual Route Mode
  {
    ...
    logo: null,
    platform: 'dual-route',
    i18nKey: 'settings.platformDualRoute',
  },

  // 自定义选项（需要用户输�?base url�? Custom option (requires user to input base url)
  { name: 'Custom', value: 'custom', logo: null, platform: 'custom', i18nKey: 'settings.platformCustom' },

  // New API 多模型网�?/ New API multi-model gateway
  {
    name: 'New API',
    value: 'new-api',
...
/**
 * 检查是否为自定义选项（无预设 base_url�? * Check if it's custom option (no preset base_url)
 */
