...
function detectComplexity(input: string): boolean {
  ...
  const complexPatterns = [
    /explain|分析|解释|为什么|how does|原理|实现|架构|设计|重构|优化/,
    /compare|对比|区别|vs\./,
    /写.*代码|编写|实现.*功能|开发|编程|写一个|coding/,
    /debug|调试|fix|修复|bug|报错|错误/,
    /多步|步骤|step.*by.*step|一步步/,
    /review|审查|检查.*代码|code.*review/,
    /翻译.*长|长.*翻译|translate.*long/,
    /generate|生成|create.*project|创建.*项目/,
    /爬虫|scraper|spider|crawl/,
    /web.*server|服务器|backend|后端|前端|frontend/,
    /database|数据库|SQL|sql/,
    /algorithm|算法/,
