...
import { Puzzle, Search, Refresh, Plus, Play, Pause, FolderOpen, Delete, Star, Download, People, CheckCircle, Shield, User, ArrowLeft } from "@icon-park/react";
...
function VerifiedBadge({ v }: { v?: string }) {
  ...
  if (v === "verified") return <Tag size="small" color="blue"><CheckCircle theme="outline" size="12" className="inline mr-2px" />{zh("Verified")}</Tag>;
  return null;
}
...
function StarRating({ rating, reviews }: { rating?: number; reviews?: number }) {
  ...
  return (
    <span className="text-12px text-[#F7BA1E] inline-flex items-center gap-2px">
      {stars}
